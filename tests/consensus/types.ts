/**
 * The output block structure that will be used to compare the OCaml consensus code with the TypeScript consensus code.
 */
...
/**
 * Types below are  used to generate the precomputed blocks that are used to run the OCaml consensus code.
 * The precomputed blocks are JSON stored in a GCP bucket, which store the data that is used to run the OCaml consensus code.
 * We transofrm the data from the JSON files into the types below, which are then used to generate the precomputed blocks.
 */
export type PrecomputedBlock = {
  version: number;
